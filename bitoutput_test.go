...
func Test_classify(t *testing.T) {
	...
	hidden := NeuLayerConfig{"sigmoid", 8}
	output := NeuLayerConfig{"softmax", nclasses}
	nn := NewNeuNetwork(input, hidden, 3, output, NeuTunables{gdalgname: RMSprop, costfname: CostCrossEntropy})
	nn.initXavier()
	maxint := int32(0xff)
	...
	converged := 0
	for converged == 0 {
		converged = nn.Train(Xs, TrainParams{resultvalcb: xorclassify, testingpct: 50, maxcost: 0.1, maxbackprops: 1E6})
	}
	if converged&ConvergedMaxBackprops > 0 {
		t.Errorf("reached the maximum number of back propagations (%d)\n", nn.nbackprops)
	}
